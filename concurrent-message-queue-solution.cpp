...
public:
    ...
    Type receive()
    {
        ...
        Type m = std::move(_messages.back());
        _messages.pop_back();

        return m; // will not be copied due to return value optimization (RVO) in C++
    }
    ...
    void send(Type &&m)
    {
        ...
        // add vector to queue
        std::cout << "   Message #" << m << " will be added to the queue" << std::endl;
...
private:
    ...
    std::deque<Type> _messages; // list of all vehicles waiting to enter this intersection
};
